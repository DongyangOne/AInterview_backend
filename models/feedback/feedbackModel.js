const db = require("../../config/database");

// backend-7: 유저별 피드백 전체 조회
const findAllByUserId = ({ userId }) => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT feedback_id AS id, title, content, memo, created_at, pin  
      FROM feedback
      WHERE userId = ?
      ORDER BY created_at DESC
    `;
    db.query(sql, [userId], (err, rows) => {
      if (err) return reject(err);
      resolve(rows);
    });
  });
};

// backend-8: 제목 조회
const findTitleById = ({ feedbackId, userId }) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT title FROM feedback WHERE feedback_id = ? AND userId = ?";
    db.query(sql, [feedbackId, userId], (err, rows) => {
      if (err) return reject(err);
      resolve(rows[0] || null);
    });
  });
};

// backend-8: 제목 수정
const updateTitle = ({ feedbackId, title, userId }) => {
  return new Promise((resolve, reject) => {
    const sql = `
      UPDATE feedback 
      SET title = ?, updated_at = NOW() 
      WHERE feedback_id = ? AND userId = ?
    `;
    db.query(sql, [title, feedbackId, userId], (err, result) => {
      if (err) return reject(err);
      resolve(result);
    });
  });
};

// backend-9: 메모 조회
const findMemoById = ({ feedbackId, userId }) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT memo FROM feedback WHERE feedback_id = ? AND userId = ?";
    db.query(sql, [feedbackId, userId], (err, rows) => {
      if (err) return reject(err);
      resolve(rows[0] || null);
    });
  });
};

// backend-9: 메모 수정
const updateMemo = ({ feedbackId, memo, userId }) => {
  return new Promise((resolve, reject) => {
    if (memo.length > 50) {
      return reject(new Error("메모는 50자 이하로 입력해주세요."));
    }
    const sql = `
      UPDATE feedback 
      SET memo = ?, updated_at = NOW() 
      WHERE feedback_id = ? AND userId = ?
    `;
    db.query(sql, [memo, feedbackId, userId], (err, result) => {
      if (err) return reject(err);
      resolve(result);
    });
  });
};

// backend-10: 검색
const searchFeedbacks = (userId, keyword) => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT feedback_id, userId, title, memo, pin, created_at
      FROM feedback
      WHERE userId = ? AND (title LIKE ? OR memo LIKE ?)
      ORDER BY created_at DESC
    `;
    db.query(sql, [userId, `%${keyword}%`, `%${keyword}%`], (err, results) => {
      if (err) return reject(err);
      resolve(results);
    });
  });
};

// backend-11: 정렬
const sortFeedbacks = (userId, orderBy) => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT feedback_id, userId, title, memo, pin, created_at
      FROM feedback
      WHERE userId = ?
      ORDER BY pin DESC, ${orderBy}
    `;
    db.query(sql, [userId], (err, results) => {
      if (err) return reject(err);
      resolve(results);
    });
  });
};

// backend-12: 상단 고정
const pinFeedback = (feedbackId, userId) => {
  return new Promise((resolve, reject) => {
    const sql =
      "UPDATE feedback SET pin = 'Y' WHERE feedback_id = ? AND userId = ?";
    db.query(sql, [feedbackId, userId], (err, result) => {
      if (err) return reject(err);
      if (result.affectedRows === 0) return resolve(null);

      const selectSql =
        "SELECT pin FROM feedback WHERE feedback_id = ? AND userId = ?";
      db.query(selectSql, [feedbackId, userId], (err2, rows) => {
        if (err2) return reject(err2);
        resolve(rows[0]);
      });
    });
  });
};

// backend-12: 고정 해제
const unpinFeedback = (feedbackId, userId) => {
  return new Promise((resolve, reject) => {
    const sql =
      "UPDATE feedback SET pin = 'N' WHERE feedback_id = ? AND userId = ?";
    db.query(sql, [feedbackId, userId], (err, result) => {
      if (err) return reject(err);
      if (result.affectedRows === 0) return resolve(null);

      const selectSql =
        "SELECT pin FROM feedback WHERE feedback_id = ? AND userId = ?";
      db.query(selectSql, [feedbackId, userId], (err2, rows) => {
        if (err2) return reject(err2);
        resolve(rows[0]);
      });
    });
  });
};

// backend-13: 삭제
const deleteById = ({ feedbackId, userId }) => {
  return new Promise((resolve, reject) => {
    const sql = `
      DELETE FROM feedback
      WHERE feedback_id = ? AND userId = ?
    `;
    db.query(sql, [feedbackId, userId], (err, result) => {
      if (err) return reject(err);
      resolve(result);
    });
  });
};

// backend-14: 피드백 조회
const findById = ({ feedbackId, userId }) => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT 
        f.feedback_id AS id, 
        f.userId, 
        f.title, 
        f.good,
        f.bad,
        f.content,
        f.memo, 
        f.pin,
        f.created_at,
        a.pose,
        a.confidence,
        a.facial,
        a.risk_response,
        a.tone,
        a.understanding
      FROM feedback AS f
      LEFT JOIN analysis AS a ON f.feedback_id = a.feedback_id
      WHERE f.feedback_id = ? AND f.userId = ?
    `;
    db.query(sql, [feedbackId, userId], (err, rows) => {
      if (err) return reject(err);
      resolve(rows[0] || null);
    });
  });
};

// 가장 최근 피드백 조회
const findPreviousFeedback = ({ userId, feedbackId }) => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT a.pose, a.confidence, a.facial, a.risk_response, a.tone, a.understanding
      FROM feedback f
      LEFT JOIN analysis a ON f.feedback_id = a.feedback_id
      WHERE f.userId = ? AND f.feedback_id < ?
      ORDER BY f.created_at DESC
      LIMIT 1
    `;
    db.query(sql, [userId, feedbackId], (err, rows) => {
      if (err) return reject(err);
      resolve(rows[0] || null);
    });
  });
};

// backend-26: 피드백 생성
const createFeedback = (data) => {
  return new Promise((resolve, reject) => {
    const feedbackSql = `
      INSERT INTO feedback (userId, title, good, bad, content, memo) 
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    const feedbackParams = [
      data.userId,
      data.title,
      data.good,
      data.bad,
      data.content,
      data.memo,
    ];

    db.query(feedbackSql, feedbackParams, (err, result) => {
      if (err) return reject(err);

      const newFeedbackId = result.insertId;

      const analysisSql = `
        INSERT INTO analysis (feedback_id, pose, confidence, facial, risk_response, tone, understanding) 
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `;
      const analysisParams = [
        newFeedbackId,
        data.pose,
        data.confidence,
        data.facial,
        data.risk_response,
        data.tone,
        data.understanding,
      ];

      db.query(analysisSql, analysisParams, (err2) => {
        if (err2) return reject(err2);
        resolve({ feedbackId: newFeedbackId });
      });
    });
  });
};

// backend-27: 피드백 수정
const updateFeedback = ({
  feedbackId,
  userId,
  good,
  bad,
  content,
  pose,
  confidence,
  facial,
  risk_response,
  tone,
  understanding,
}) => {
  return new Promise((resolve, reject) => {
    const feedbackSql = `
      UPDATE feedback 
      SET good = ?, bad = ?, content = ?, updated_at = NOW() 
      WHERE feedback_id = ? AND userId = ?
    `;
    const feedbackParams = [good, bad, content, feedbackId, userId];

    db.query(feedbackSql, feedbackParams, (err) => {
      if (err) return reject(err);

      const analysisSql = `
        UPDATE analysis 
        SET pose = ?, confidence = ?, facial = ?, risk_response = ?, tone = ?, understanding = ?
        WHERE feedback_id = ?
      `;
      const analysisParams = [
        pose,
        confidence,
        facial,
        risk_response,
        tone,
        understanding,
        feedbackId,
      ];

      db.query(analysisSql, analysisParams, (err2, analysisResult) => {
        if (err2) return reject(err2);
        resolve(analysisResult);
      });
    });
  });
};

module.exports = {
  findAllByUserId,
  findTitleById,
  updateTitle,
  findMemoById,
  updateMemo,
  searchFeedbacks,
  sortFeedbacks,
  pinFeedback,
  unpinFeedback,
  deleteById,
  findById,
  createFeedback,
  updateFeedback,
  findPreviousFeedback,
};

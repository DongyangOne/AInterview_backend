...
exports.deleteFeedback = async (req, res) => {
  try {
    ...
    const result = await feedbackModel.deleteById({ feedbackId, userId });
...
exports.getFeedbackDetail = async (req, res) => {
  try {
    ...
    const feedback = await feedbackModel.findById({ feedbackId, userId });

    if (!feedback) {
      return res.status(404).json({
        success: false,
        message: '해당 ID의 피드백을 찾을 수 없거나 조회 권한이 없습니다.'
      });
    }

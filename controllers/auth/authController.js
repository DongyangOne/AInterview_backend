...
const signinProgress = (req, res) => {
    ...
    if (!loginUserId) {
        //console.log('아이디 미입력');
        console.log(`${requestTime} 로그인 ${400} 응답`);
    ...
    loginCheck(loginUserId, password, (err, result) => {
        if (err) {
            switch (err.code) { //각종 에러 처리
                case 'DB_ERROR':
                    console.log(`${requestTime} 로그인 ${500} 응답`);
                    return res.status(500).json({ success: false, message: err.message });
                case 'USER_NOT_FOUND':
                    console.log(`${requestTime}] 로그인 ${404} 응답`);
                    return res.status(404).json({ success: false, message: err.message });
                case 'INVALID_PASSWORD':
                    console.log(`${requestTime} 로그인 ${401} 응답`);
                    return res.status(401).json({ success: false, message: err.message });
            }
        }
        console.log(`${requestTime} 로그인 ${200} 응답`);
        return res.status(200).json({ success: true, loginUserId: result[0].user_id, userId : result[0].id, nickname: result[0].nickname, message: '로그인 상태' });
    })
}
...
const signupProgress = (req, res) => {
    ...
    if (inputErrors.length > 0) {
        ...
        console.log(`${requestTime} 회원가입 ${400} 응답`);
    ...
    userIdCheck(loginUserId, (err, result) => {
        if (err) {
            switch (err.code) {
                case 'DB_ERROR':
                    console.log(`${requestTime} 회원가입 ${500} 응답`);
                    return res.status(500).json({ success: false, message: err.message });
                case 'ID_DUPLICATE':
                    console.log(`${requestTime} 회원가입 ${409} 응답`);
            ...
            addUser(loginUserId, nickname, password, appPush, (err, result) => {
                if (err) {
                    ...
                    console.log(`${requestTime} 회원가입 ${201} 응답`);
...
const userIdCheckProgress = (req, res) => {
    ...
    if (!loginUserId) {
        //console.log('아이디 미입력');
        console.log(`${requestTime} 아이디 중복확인 ${400} 응답`);
        return res.status(400).json({ success: false, message: '미입력 정보가 존재합니다.' });
    }
    ...
    userIdCheck(loginUserId, (err, result) => {
        if (err) {
            switch (err.code) {
                case 'DB_ERROR':
                    console.log(`${requestTime} 아이디 중복확인 ${500} 응답`);
                    return res.status(500).json({ success: false, message: err.message });
                case 'ID_DUPLICATE':
                    console.log(`${requestTime} 아이디 중복확인 ${409} 응답`);
                    return res.status(409).json({ success: false, message: err.message });
            }
        }
        else {
            console.log(`${requestTime} 아이디 중복확인 ${200} 응답`);
